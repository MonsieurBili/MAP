...
public class DuckFactory implements UserFactory<Duck> {
    ...
    public void setData(String username,String email,String password,TipRata tipRata, double viteza, double rezistenta)
    {
    ...
    public Duck createUser() {
        if (tipRata == TipRata.SWIMMING)
            return new SwimmingDuck(username,email,password,tipRata,viteza, rezistenta);
        if (tipRata == TipRata.FLYING)
            return new FlyingDuck(username,email,password,tipRata,viteza,rezistenta);
        if (tipRata == TipRata.FLYING_AND_SWIMMING)
            return new FlyingAndSwimmingDuck(username,email,password,tipRata,viteza,rezistenta);
        return null;
    }
}
